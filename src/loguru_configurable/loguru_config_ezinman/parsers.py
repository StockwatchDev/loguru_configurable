"""This module contains functions to parse different types of configuration values."""
...
def parse_reference(reference_object: Mapping[str | int, Any] | Sequence[Any] | Any, ref: str) -> Any:
    """
    ...
    while len(rest) > 0:
        ...
        if isinstance(current, (list, tuple)):
            try:
                ...
                raise KeyError(f"Invalid reference: {ref!r}.") from exc
        else:
            ...
            try:
                ...
                if ref.isdigit():
                    current = current[int(ref)]  # type: ignore[index]
                else:
                    raise KeyError(f"Invalid reference: {ref!r}.") from exc
    return current
...
def parse_external(external_ref: str) -> Any:
    ...
    try:
        ...
        v = ValueError(f"Cannot resolve {external_ref}: {e}")
        raise v from e
...
def parse_user_defined(
    user_defined_dict: dict[str, Any], further_parsing_function: Callable[[Any], Any] | None = None
) -> Any:
    """
    ...
    """
    if (calling_function := user_defined_dict.pop("()", _missing)) is _missing:
